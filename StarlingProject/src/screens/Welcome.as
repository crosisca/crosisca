package screens
{
	...
	public class Welcome extends Sprite
	{
		...
		private function drawScreen():void
		{
			...
			// = new Button(Assets.getTexture("WelcomePlayBtn"));
		...
		private function heroAnimation(event:Event):void
		{
			...
			hero.y = 100 + (Math.cos(currentDate.getTime() * 0.002) * 25);//0.002 = speed, 125-175(posY)
			playBtn.y = 260 + (Math.cos(currentDate.getTime() * 0.002) * 10);//0.002 = speed, 125-175(posY)
			aboutBtn.y = 380 + (Math.cos(currentDate.getTime() * 0.002) * 10);//0.002 = speed, 125-175(posY)
		}
	}
}