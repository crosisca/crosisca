package citrus.objects.complex.box2dstarling
{
	 ...
	 */
	public class BambooFountain extends Box2DPhysicsObject
	{
		...
		private var axleFriction:Number = 15;
		...
		public function BambooFountain(name:String, params:Object = null)
		{
		...
		private function makeCar():void {
			...
			var vertices:Vector.<b2Vec2> = new Vector.<b2Vec2>();
			 ...
			 vertices.push(new b2Vec2(-58/_box2D.scale, -7/_box2D.scale));
					vertices.push(new b2Vec2(-74/_box2D.scale, 9/_box2D.scale));
					vertices.push(new b2Vec2(-74/_box2D.scale, -52/_box2D.scale));
					vertices.push(new b2Vec2(-58/_box2D.scale, -52/_box2D.scale));
					
					_tab.push(vertices);
					vertices = new Vector.<b2Vec2>();
											
			        vertices.push(new b2Vec2(-58/_box2D.scale, -7/_box2D.scale));
					vertices.push(new b2Vec2(96/_box2D.scale, -7/_box2D.scale));
					vertices.push(new b2Vec2(96/_box2D.scale, 9/_box2D.scale));
					vertices.push(new b2Vec2(-74/_box2D.scale, 9/_box2D.scale));
			...
			carShape.SetAsBox(3 / worldScale, 3 / worldScale);
			...
			carBodyDef.type = b2Body.b2_staticBody;
			...
			//leftAxleShape.SetAsOrientedBox(100 / worldScale / 2, 8 / worldScale, new b2Vec2(0, 0),0);
			...
			for (var i:uint = 0; i < _tab.length; ++i) {
				...
				polygonShape.SetAsVector(_tab[i], 0)
				leftAxleFixture.shape = polygonShape;
				leftAxle.CreateFixture(leftAxleFixture);
				addBody(leftAxle);
			}
			/*leftAxle.SetAngle( -10 * degreesToRadians); // dont set angle manually --- it behaves badly!
			leftAxle.SetAngularDamping(1);*/
			//leftAxle.SetPosition(new b2Vec2(carPosX/ws - 128 /ws, carPosY/ws - 48/ws));
			

			
			//leftAxle.GetWorldPoint(new b2Vec2(32/ws,64/ws))
			//leftAxle.GetWorldCenter().add2(new b2Vec2(32/ws,32/ws))
			var leftWheelRevoluteJointDef:b2RevoluteJointDef = new b2RevoluteJointDef();
			leftWheelRevoluteJointDef.Initialize(leftAxle, car, leftAxle.GetWorldCenter());
			
			//trace (leftAxle.GetWorldCenter().x, leftAxle.GetWorldCenter().y);
			leftWheelRevoluteJointDef.localAnchorA = new b2Vec2(0,0);
			leftWheelRevoluteJointDef.localAnchorB = new b2Vec2(0,0);
			
			
			
			//trace (leftAxle.GetWorldCenter().x * ws, leftAxle.GetWorldCenter().y * ws);
			//leftWheelRevoluteJointDef.enableMotor = true;
			leftWheelRevoluteJointDef.enableLimit = true;
			leftWheelRevoluteJointDef.lowerAngle = -10 * degreesToRadians;
			leftWheelRevoluteJointDef.upperAngle = 5 * degreesToRadians;
			...
			//leftWheelRevoluteJoint.SetLimits( -5 * degreesToRadians, 5 * degreesToRadians);
			
			/*var anchorA:b2Vec2 = new b2Vec2(leftWheelRevoluteJoint.GetAnchorA().x * ws, leftWheelRevoluteJoint.GetAnchorA().y * ws);
			
			anchorA.add2(32/ws, 32/ws);
			leftWheelRevoluteJoint.GetAnchorA() = anchorA;*/
		
			
			
			/*var rightAxleShape:b2PolygonShape = new b2PolygonShape();
			rightAxleShape.SetAsOrientedBox(16 / worldScale / 2, 40 / worldScale, new b2Vec2(0, 0), 0 * degreesToRadians);
			...
			rightAxle.SetPosition(new b2Vec2(carPosX / worldScale -50 / worldScale , carPosY / worldScale));*/
			
			/*var distanceJointDef:b2DistanceJointDef = new b2DistanceJointDef();
			distanceJointDef.Initialize(rightAxle, car, rightAxle.GetWorldCenter(), car.GetWorldCenter().subtract2(new b2Vec2(1, 0)));
			distanceJointDef.length = 1;
			var distanceJoint:b2DistanceJoint = _box2D.world.CreateJoint(distanceJointDef) as b2DistanceJoint;*/
